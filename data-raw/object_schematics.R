...
celln    [label = 'cell N\n(pt2cell)' class = 'grvzclick'
          URL = '#cell-objects-pt2cell'];
note     [label = 'note\n(character)' group = 'cell'];
instr    [label = 'instrument\n(integer)' group = 'cell'];
eff      [label = 'effect command\n(pt2command)' group = 'cell' class = 'grvzclick'
          URL = '#command-objects-pt2command'];
...
") |>
    ...
    read_xml() |>
    as.character()
  svgdata <- gsub("scale(1 1)", "scale (0.6 0.6)", svgdata, fixed = TRUE)
  svgdata <- gsub("height=\"314pt\"", "height=\"230pt\"", svgdata, fixed = TRUE)
  svgdata <- gsub("viewBox=\"0.00 0.00 968.00 314.00\"", "viewBox=\"0.00 0.00 968.00 230.00\"",
                  svgdata, fixed = TRUE)
