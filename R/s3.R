...
format.pt2pat <- function(x, padding = " ", empty_char = "-", fmt = getOption("pt2_cell_format"), ...) {
  if (is.null(fmt)) {
    ...
    if (requireNamespace("crayon", quietly = TRUE)) {
      ...
      if (is.null(fmt_note)) fmt_note <- c(crayon::blue("%s"), crayon::silver("%s"))
      if (is.null(fmt_inst)) fmt_inst <- crayon::cyan("%02i")
      if (is.null(fmt_efft)) fmt_efft <- c(crayon::green("%X%02X"), crayon::silver("%X%02X"))
      if (is.null(fmt_padd)) fmt_padd <- crayon::silver("%s")
